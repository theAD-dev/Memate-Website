    ...
    if (supplierData) {
      reset({
        ...
        industry: supplierData.supplier_industries_id
          ? { label: supplierData.industry, value: supplierData.supplier_industries_id }
          : null,
        supplied_services: supplierData.supplier_services_id
          ? supplierData.supplier_services_id.split(',').map(id => ({ label: `Service ${id}`, value: parseInt(id) }))
          : []
      });
    }
  }, [supplierData]);
...
useEffect(() => {
  const fetchAndSetIndustries = async () => {
    ...
    if (supplierData) {
      const selectedIndustry = formattedOptions.find(opt => opt.value === Number(supplierData.supplier_industries_id));
      ...
      if (selectedIndustry) {
        const services = await fetchServicesByIndustryId(selectedIndustry.value);
        setServicesOptions(services.map(s => ({
          label: s.title,
          value: s.id
        })));
      }
  ...
  const onSubmit = async (data) => {
    const requestData = {
      ...
      supplier_services_id: data.supplied_services.map(s => s.value).join(','),
    };
  ...
  return (
    <>
      ...
      >
         <div className={`uploadEditWrap ${style.supplierAddCompany} ${style.supplierRequestEdit}`}>
            ...
            <Controller
              ...
              render={({ field }) => (
                <textarea
                  ...
                  {...field}
                  rows={6}
                  style={{ width: "100%", padding: "10px", border: "1px solid #D0D5DD" }}
                />
              )}
            />
          </div>
